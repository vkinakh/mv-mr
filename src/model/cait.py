...
class CaiT(nn.Module):
    ...
    def __init__(self, img_size: int = 224, patch_size: int = 16,
                 ...
                 qkv_bias: bool = False, qk_scale=None,
                 drop_rate: float = 0., attn_drop_rate=0.,
...
class CaiTMultiProj(nn.Module):

    """CaiT with projector"""

    def __init__(self, out_dim: Union[str, int], patch_size: int, img_size: int, **kwargs):
        super().__init__()

        print(f'Creating CaiT model with output dimension: {out_dim}')
        self.backbone = CaiT(img_size=img_size, patch_size=patch_size, num_classes=0,
                             norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
        if isinstance(out_dim, int):
            sizes = [self.backbone.num_features, out_dim]
        else:
            sizes = [self.backbone.num_features] + list(map(int, out_dim.split('-')))
...
if __name__ == '__main__':
    ...
    h, z = model(torch.randn(16, 3, h, w))
