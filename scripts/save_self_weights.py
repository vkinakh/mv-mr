...
from src.model import SelfSupervisedModule
...
def save_encoder_weights(input_path):
    ...
    for ckpt_file in tqdm(ckpt_files):
        ...
        # load module
        module = SelfSupervisedModule.load_from_checkpoint(checkpoint_path=ckpt_file,
                                                           config=config)
        ...
        torch.save(
            {
                ...
                'online_finetuner': module.online_finetuner.state_dict()
            },
            output_path
        )
